...
# Load data 
pred_agg_firm <- read.fst("pat_pred_top_startups_16_0.9.fst") 
pred_agg_firm <- filter(pred_agg_firm, model %in% c("past", "C_3_model_2", "C_3_model_3", "C_3_model_4", "C_3_model_5", "C_3_prob", "model_three"))
pred_agg_firm <- mutate(pred_agg_firm, country_name = countrycode(country, "iso2c", "country.name.en"))
...
server <- function(input, output) {
    ...
    output$firm_pat_plot <- renderPlotly({
        if(nrow(dat_set())!= 0){   
            ...
            p <-  ggplotly(
                ggplot(data = filter(dat_set()), aes(x = as.factor(model), y = rel_class, color = as.factor(organization), shape = as.factor(model), group = as.factor(interaction(organization, model)), label = info)) +
                    ...
                    geom_point(data = filter(dat_set(), pub_year > 2016), aes(x = as.factor(model), y = rel_class, color = as.factor(organization), shape = as.factor(model), group = as.factor(interaction(organization, model))), size = 2) +
